...
function hasExplicitMacros_(query){
  const q = String(query||"").toLowerCase();
  // Detects patterns like "45g protein", "protein 45g", "500 kcal", etc.
  return /\b\d+(\.\d+)?\s*(g|gm|grams?)\s*(protein|carbs?|fats?|fiber)\b/.test(q) ||
         /\b(protein|carbs?|fats?|fiber)\s*\d+(\.\d+)?\s*(g|gm|grams?)\b/.test(q) ||
         /\b\d+(\.\d+)?\s*(kcal|calories?|cals?)\b/.test(q) ||
         /\b(kcal|calories?|cals?)\s*\d+(\.\d+)?\b/.test(q);
}

function hasPortionDescriptor_(query){
  const q = String(query || "").toLowerCase();
  return /\d+\s*(katori|bowl|plate|cup|piece|slice|roti|paratha|chapati|glass|scoop|tbsp|tablespoon|tsp|teaspoon|serving|pcs?|nos?|g\b|gm\b|gram|ml\b|oz\b|small|medium|large|half)/.test(q)
    || /\b(a |one |two |three |four |half a?)\s*(katori|bowl|plate|cup|piece|slice|roti|paratha|chapati|glass|scoop|serving)/i.test(q);
}
...
function groqEstimateNutrients_(foodText, opts){
  ...
  const hasExplicit = (opts && opts.hasExplicitMacros);
  ...
  const sys = [
    ...
    "",
    "CRITICAL RULES:",
    "1. EXPLICIT MACROS (HIGHEST PRIORITY): If the user's text includes exact nutritional values (e.g., '45g protein', '149g carbs', '42g fat', '500 kcal'), YOU MUST USE THOSE EXACT VALUES for calories_kcal, protein_g, carbs_g, fat_g, fiber_g. Do not scale or alter them. Only estimate the micronutrients that would realistically accompany those macros.",
    "2. If no explicit macros are given, but a portion is (e.g., '1 katori', '2 rotis'), estimate nutrients for THAT EXACT PORTION.",
    "3. First estimate the serving weight in grams (estimated_serving_g).",
    "4. For Indian foods: 1 katori ≈ 150-180g, 1 roti ≈ 40g, 1 plate rice ≈ 180-200g, 1 bowl dal ≈ 150-180g.",
    "5. For BRANDED/PACKAGED foods: Use the exact nutrition label data. Serving size is printed on the pack.",
    "6. SANITY CHECK: calories must roughly equal (protein_g × 4) + (carbs_g × 4) + (fat_g × 9). Skip this check ONLY if the user provided explicit macros that differ.",
  ...
  // Skip the auto-correction sanity check if the user manually provided the macros!
  if(!hasExplicit) {
...
const INDIAN_FOOD_REF_ = {
  ...
  // ── Snacks & sides ──
  // ── Snacks & sides ──
  "chicken momo":        {calories_kcal:164, protein_g:7.4, carbs_g:17, fat_g:7.3, fiber_g:1, micros:{sodium_mg:350, potassium_mg:150}},
...
function llmEstimatePortionGrams_(foodDescription){
  // Switched from getGroqNutrientModel_() to the blazing fast 8B model
  ...
  const sys = [
    ...
    "- dosa: 80-120g, idli: 35-45g each, momo: 21.2g each",
...
function estimateNutrientsCached_(dishKey, query){
  ...
  // Check if the user manually provided macros/calories
  const hasMacros = hasExplicitMacros_(q);
...
// --- STEP 2: For non-Indian foods, try USDA FDC ---
    // Fast-fail: USDA database cannot handle multi-item combinations. Skip to LLM.
    const looksComposite = /(\+| and | with |&|,|\bmomo\b|\btikka\b|\bsushi\b|\bpizza\b|\bpasta\b|\bburger\b|\broll\b|\bsandwich\b)/i.test(q);
    ...
    if (!looksComposite) {
      ...
      try {
        ...
        if (meta.dataType === "Branded" && meta.servingSize > 0) {
          ...
          const explicitG = extractExplicitGrams_(q);
          if (explicitG) nutrients = scaleFdcToServing_(nutrientsPer100g, explicitG);
        }
        ...
        // FDC failed - continue to LLM (Step 3)
      }
    }
  }
  ...
  try{
    const llm = groqEstimateNutrients_(q, {
      useWebSearch: !hasMacros, 
      hasExplicitMacros: hasMacros
    });
    if(llm && typeof llm === "object"){
      setCachedFoodNutrients_(key, q, {fdcId:"", description: hasMacros ? "LLM estimate (explicit macros)" : "LLM estimate", dataType:"llm"}, llm);
      return llm;
    }
  }catch(_ignoreLlmErr){}
...
function enrichWithLlm_(foodText, refPer100g){
  ...
  const sys = [
    ...
    "   dosa 80-120g, idli 35-45g each, momo 21g each, plate rice 180-220g, bowl dal 150-200g, glass 200ml.",
...
function upsertDay_(dateKey, payloadStr, {source="manual", email=""}={}){
  ...
  try{
    ...
    const row = buildRowForSheet_(dateKey, mergedPayload, summary, {
      source,
      email,
      rev: Math.max(latestRev, toNum_(mergedPayload.rev)) + 1,
...
function buildRowForSheet_(dateKey, payloadObj, summary, {source="manual", email="", rev=0, deviceId=""}={}){
  ...
  row["Calories Burnt (kcal)"] = toNum_(payloadObj.calBurned);
  row["Net Calories (kcal)"] = Math.round((summary.cal - toNum_(payloadObj.calBurned))*10)/10;

  row["Water Intake (mL)"] = summary.waterMl;
  row["Steps"] = summary.steps;
  row["Sleep (hrs)"] = toNum_(payloadObj.sleep && payloadObj.sleep.hours);

  row["Weight (kg)"] = toNum_(payloadObj.weight);
  row["BMI"] = toNum_(payloadObj.bmi);
  ...
  row["Calcium (mg)"] = toNum_(micros.calcium_mg);
  row["Iron (mg)"] = toNum_(micros.iron_mg);
  row["Potassium (mg)"] = toNum_(micros.potassium_mg);
  row["Sodium (mg)"] = toNum_(micros.sodium_mg);
  row["Magnesium (mg)"] = toNum_(micros.magnesium_mg);
  row["Zinc (mg)"] = toNum_(micros.zinc_mg);
  row["Vitamin A (mg)"] = toNum_(micros.vitamin_a_mg);
  row["Vitamin C (mg)"] = toNum_(micros.vitamin_c_mg);
  row["Vitamin D (mg)"] = toNum_(micros.vitamin_d_mg);
  row["Vitamin B12 (mg)"] = toNum_(micros.vitamin_b12_mg);
...
function computeSummaryFromPayload_(p){
  ...
  foods.forEach(f=>{
    cal += toNum_(f.cal);
    protein += toNum_(f.protein);
    carbs += toNum_(f.carbs);
    fat += toNum_(f.fat);
    fiber += toNum_(f.fiber);
    if(f.micros && typeof f.micros==="object"){
      Object.keys(f.micros).forEach(k=>{
        micros[k] = (micros[k]||0) + toNum_(f.micros[k]);
      });
    }
  });
  ...
  taken.forEach(id=>{
    const sn = suppMap[id];
    if(!sn) return;
    cal += toNum_(sn.calories_kcal);
    protein += toNum_(sn.protein_g);
    carbs += toNum_(sn.carbs_g);
    fat += toNum_(sn.fat_g);
    fiber += toNum_(sn.fiber_g);
    if(sn.micros && typeof sn.micros==="object"){
      Object.keys(sn.micros).forEach(k=>{
        micros[k] = (micros[k]||0) + toNum_(sn.micros[k]);
      });
    }
  });

  const waterMl = waterLogs.reduce((s,l)=>s + toNum_(l.ml), 0);
  const steps = stepLogs.reduce((mx,l)=>Math.max(mx, Math.round(toNum_(l.steps))), 0);
...
function mergeArrayById_(aArr, bArr, idKey, tsKey){
  ...
  b.forEach(x=>{
    ...
    const t1 = toNum_(cur[tsKey]);
    const t2 = toNum_(x[tsKey]);
...
function mergeLwwMap_(a,b){
  ...
  keys.forEach(k=>{
    ...
    const tx = (x && typeof x==="object") ? toNum_(x.ts) : 0;
    const ty = (y && typeof y==="object") ? toNum_(y.ts) : 0;
    out[k] = (ty >= tx) ? y : x;
  });
  return out;
}
