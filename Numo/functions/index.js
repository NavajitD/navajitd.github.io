...
- End with "### Things you can do" — 2-4 bold bullets, each with a one-sentence "why".
  ...
  (req, res) => {
    corsHandler(req, res, async () => {
      ...
      try {
        ...
        // Verify auth token if present; load authoritative profile from Firestore.
        // This prevents profile spoofing for authenticated users.
        ...
        if (authHeader && authHeader.startsWith("Bearer ")) {
          try {
            ...
            uid = decoded.uid;
            const profileDoc = await db.collection("numo_users").doc(uid).get();
            ...
            console.warn(`[${requestId}] auth token invalid, proceeding as guest: ${e.message}`);
          }
        }
...
// Admin emails that can overwrite a locked profile
const ADMIN_EMAILS = ["navjitdebnath5@gmail.com", "sankhanadh@gmail.com"];

/**
 * Save (or update for admins) a user's numerology profile.
 * POST /saveProfile  —  Authorization: Bearer <idToken>
 * Body: { name, dob, gender }
 *
 * Normal users: profile is immutable after first save (403 on re-attempt).
 * Admin emails: can overwrite at any time.
 */
exports.saveProfile = onRequest(
  ...
  (req, res) => {
    corsHandler(req, res, async () => {
      ...
      if (!name || !dob || !gender) {
        res.status(400).json({ error: "name, dob, and gender are required" });
        return;
      }

      const profileRef = db.collection("numo_users").doc(uid);
      const existing = await profileRef.get();
      const isAdmin = ADMIN_EMAILS.includes(email);

      if (existing.exists && !isAdmin) {
        res.status(403).json({ error: "Profile is locked and cannot be changed." });
        return;
      }
      ...
      console.info(`[saveProfile] uid=${uid} email=${email} isAdmin=${isAdmin} overwrite=${existing.exists}`);
